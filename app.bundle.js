/* Infinite Paths v0.2.2 | generated from modular source | do not edit this bundle directly */
...
(function (root, factory) {
  ...
  const APP_CONFIG = Object.freeze({
    name: 'Infinite Paths',
    version: '0.2.2',
    buildId: 'personal-profile-content-pack-hotfix-2026-08-02',
    milestone: 'Milestone 2A: Campaign Identity and Creation',
    rulesVersion: '0.2.2',
    contentSchemaVersion: 2,
    contentVersion: 'ip.original.horror-foundation@0.1.1',
...
(function (root, factory) {
  ...
  const BUILTIN_CONTENT_PACKS = Object.freeze([
    Object.freeze({
      ...
      version: '0.1.1',
      description: 'The original deterministic Horror vertical-slice foundation for Infinite Paths.',
      kind: 'original',
      delivery: 'embedded',
  ...
  function requireUniqueStringArray(value, label, allowedValues = null, minimum = 0) {
    ...
    const normalized = value.map((item) => requireText(item, label, 160));
  ...
  function normalizeFiles(value, delivery) {
    ...
    return value.map((file) => {
      ...
      if (!FILE_TYPES.includes(normalized.type)) throw new RangeError(`Unsupported content file type: ${normalized.type}`);
      if (!SAFE_JSON_PATH_PATTERN.test(normalized.path)) throw new RangeError('Content file path must be a safe relative JSON path.');
      if (!['startup', 'campaign-create', 'act', 'lazy'].includes(normalized.loadPhase)) throw new RangeError('Content file load phase is invalid.');
  ...
  function isContentPackCompatibleWithPlayerProfile(candidate, profile = IP.APP_CONFIG?.playerProfile) {
    ...
    return sexes.includes(profile.sex) && desiredPartners.every((sex) => romanceSexes.includes(sex));
  }
...
(function (root, factory) {
  ...
  function generateHorrorWorld(seed, contentPack = IP.getBuiltInContentPack()) {
  ...
  function validateCampaignInput(input) {
    ...
    const contentPack = IP.getBuiltInContentPack(input?.contentPackId || IP.APP_CONFIG.defaultContentPackId);
    if (!contentPack.genres.includes('horror')) {
      throw new RangeError('The selected content pack does not provide the Horror generator.');
    }
    ...
    return { name, background: { ...background }, difficulty, length, seed, genre: 'horror', contentPack };
  }

  function generateCampaign(input, now = () => new Date().toISOString()) {
    const settings = validateCampaignInput(input);
    const createdAt = now();
    const world = generateHorrorWorld(settings.seed, settings.contentPack);
    const id = campaignId(settings.seed, createdAt, settings.contentPack);
    const background = settings.background;
    const protagonist = { name: settings.name, ...copyPlayerProfile(), background };
    ...
    const save = {
      ...
      checksum: IP.SeededRandom.fingerprint(JSON.stringify(campaign)),
      payload: {
        campaignId: id,
        status: 'created',
        contentPack: { id: campaign.contentPack.id, version: campaign.contentPack.version },
        player: { protagonist: cloneData(protagonist) },
        worldState: {},
        currentScene: null,
        eventHistory: [],
        inventory: [background.resource],
        injuries: [],
        clues: [],
        relationships: {},
      },
    ...
    const archive = {
      ...
      contentPackId: campaign.contentPack.id,
      contentPackVersion: campaign.contentPack.version,
      status: 'active',
      outcome: null,
      createdAt,
      updatedAt: createdAt,
...
(function (root, factory) {
  ...
  function renderHomeView(container, context) {
    ...
    const statusGrid = IP.h('section', { className: 'status-grid', attrs: { 'aria-label': 'Application readiness' } },
      ...
      statusTile('Content pack', 'Horror Foundation 0.1.1', 'ready')
    );
...
(function (root, factory) {
  ...
  function renderCreateCampaignView(container, context) {
    ...
    const form = IP.h('form', { className: 'card campaign-form', on: { submit: async (event) => {
      ...
      try {
        const bundle = IP.generateCampaign({ name: name.value, difficulty: difficulty.value, length: length.value, background: background.value, seed: seed.value, graphicViolence: document.getElementById('graphic-violence').checked, psychologicalHorror: document.getElementById('psychological-horror').checked });
      ...
      field('Protagonist name', name), field('Background', background),
      field('Difficulty', difficulty, 'Brutal mode will never add plot armor.'), field('Campaign length', length),
      field('World seed', seed, 'The seed, content-pack version, and rules version permanently define campaign truth.'),
      ...
      error, submit);
    container.append(IP.h('header', { className: 'view-header' }, IP.h('p', { className: 'eyebrow', text: 'CAMPAIGN CREATION' }), IP.h('h1', { id: 'page-title', text: 'Establish a world that cannot change its story afterward.' }), IP.h('p', { text: 'Creation permanently locks the seed, rules, content pack, antagonist, hidden truth, factions, geography, and ending conditions.' })), form);
  }
...
(function (root, factory) {
  ...
  async function renderCampaignView(container, context) {
    ...
    container.append(
      IP.h('section', { className: 'hero-card campaign-hero' }, IP.h('p', { className: 'eyebrow', text: 'LOCKED CAMPAIGN' }), IP.h('h1', { id: 'page-title', text: campaign.title }), IP.h('p', { className: 'hero-card__lead', text: `${campaign.protagonist.name}, a ${campaign.protagonist.background.name.toLowerCase()}, has entered ${world.region}. The facts below were established at creation and cannot be rewritten by later choices.` }), IP.h('div', { className: 'chip-row' }, IP.h('span', { className: 'chip', text: 'Horror' }), IP.h('span', { className: 'chip', text: campaign.difficulty }), IP.h('span', { className: 'chip', text: campaign.length }), IP.h('span', { className: 'chip', text: campaign.contentPack?.title || 'Horror Foundation' }))),
      IP.h('section', { className: 'card world-grid' }, fact('Seed', campaign.seedFingerprint), fact('Settlement', world.settlement), fact('Threat', world.antagonist.name), fact('Rules', campaign.rulesVersion), fact('Content pack', `${campaign.contentPack?.title || 'Horror Foundation'} ${campaign.contentPack?.version || '0.1.1'}`), fact('Status', 'World established')),
